...
static NSString *identifier = @"cellLable";

#define KWidth [UIScreen mainScreen].bounds.size.width
#define KHeight [UIScreen mainScreen].bounds.size.height

@interface SearchViewController ()<UITextFieldDelegate,BMKGeoCodeSearchDelegate,BMKPoiSearchDelegate,SearchResultViewControllerDelegate,TestViewControllerDelegate,UICollectionViewDataSource,UICollectionViewDelegate>
...
{
//    static NSString *inden = @"cellLable";
...
//    [backBtn addTarget:self action:@selector(backBUttClick) forControlEvents:UIControlEventTouchUpInside];
    ...
    CGFloat textFieldW = KWidth * 0.65;
    UITextField *textField = [[UITextField alloc] initWithFrame:CGRectMake(0, 0, textFieldW, 28)];
...
- (void)tureBtnClick{
    ...
    //        [self.delegate searchWithtext:self.textField.text];
    //    }
    [self.textField resignFirstResponder];
    //    [[NSNotificationCenter defaultCenter] postNotificationName:@"searchTure" object:self.textField.text];
...
- (void)searchBegan2{
    //反地理编码
    //    NSString *mySearch = noti.object;//将通知内容传递
...
- (void)searchCity{
    ...
    citySearchOption.pageIndex = self.curPage;
    citySearchOption.pageCapacity = 20;
    citySearchOption.city = self.myLoc;
    citySearchOption.keyword = self.mySearch;
...
- (void)onGetPoiResult:(BMKPoiSearch *)searcher result:(BMKPoiResult *)poiResult errorCode:(BMKSearchErrorCode)errorCode{
    if (errorCode == BMK_SEARCH_NO_ERROR) {
        ...
        for (int i = 0; i < poiResult.poiInfoList.count; i++) {
            ...
            NSLog(@"%@",poiInfos.address);

        }
        
//        SearchResultViewController *searchRes = [[SearchResultViewController alloc] init];
        TestViewController *test = [[TestViewController alloc] init];
        test.userLocation = self.userLocation;
        if (self.curPage != 0) {
//            searchRes.infoData = self.infoDatas;
            test.infoData = self.infoDatas;
            return;
        }
        
        //结果显示
//        searchRes.delegate = self;
//        searchRes.title = self.textField.text;
//        searchRes.infoData = self.infoDatas;
        test.delegate = self;
        test.title = self.mySearch;
        test.infoData = self.infoDatas;
        NSLog(@"%@",test.infoData);
//        NSLog(@"%@",searchRes.infoData);
...
//弹窗提示搜索错误
- (void)alertView{
    UIAlertController *aler = [UIAlertController alertControllerWithTitle:@"提示" message:@"抱歉，未找到结果" preferredStyle:UIAlertControllerStyleAlert];
